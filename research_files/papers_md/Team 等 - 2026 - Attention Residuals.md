...
![](images/c327ebe4cc92e5e282cc01b99d2d6ab9a526853a41cf3a79429163c1e83d7e1c.jpg)  
(a) Standard Residuals

![](images/7c099d913634e5941c9574526a75c5c8f23fe5455d87f7e1e7d193b7956f5a06.jpg)  
(b) Full Attention Residuals

![](images/004ecfa291f98c077d3baa79f618df5f75b9ecc3934882345601b21c87697bc9.jpg)  
...
$$
\boldsymbol {h} _ {l} = \left(1 - \boldsymbol {g} _ {l}\right) \odot \boldsymbol {h} _ {l - 1} + \boldsymbol {g} _ {l} \odot f _ {l - 1} (\boldsymbol {h} _ {l - 1})
$$
...
$$
\alpha_ {i \rightarrow l} = \frac {\phi (\boldsymbol {q} _ {l} , \boldsymbol {k} _ {i})}{\sum_ {j = 0} ^ {l - 1} \phi (\boldsymbol {q} _ {l} , \boldsymbol {k} _ {j})}\tag{2}
$$

For each layer $l ,$ we define:

$$
\boldsymbol {q} _ {l} = \boldsymbol {w} _ {l}, \qquad \boldsymbol {k} _ {i} = \boldsymbol {v} _ {i} = \left\{ \begin{array}{l l} \boldsymbol {h} _ {1} & i = 0 \\ f _ {i} (\boldsymbol {h} _ {i}) & 1 \leq i \leq l - 1 \end{array} \right.\tag{3}
$$
...
$$
\boldsymbol {b} _ {n} = \sum_ {j \in \mathcal {B} _ {n}} f _ {j} (\boldsymbol {h} _ {j})\tag{5}
$$
...
$$
\mathbf {V} = \left\{ \begin{array}{l l} [ \boldsymbol {b} _ {0}, \boldsymbol {b} _ {1}, \ldots , \boldsymbol {b} _ {n - 1} ] ^ {\top} & \text { if } i = 1 (\text { first   layer   of   block } n) \\ [ \boldsymbol {b} _ {0}, \boldsymbol {b} _ {1}, \ldots , \boldsymbol {b} _ {n - 1}, \boldsymbol {b} _ {n} ^ {i - 1} ] ^ {\top} & \text { if } i \geq 2 (\text { subsequent   layers }) \end{array} \right.\tag{6}
$$
...
![](images/441166633c1b0321e564d685d2ebdfb39bc2e69e5131e74f7c81b12828f5df38.jpg)  
...
Pipeline communication. With standard residual connections, pipeline parallelism [18] transfers a fixed-size hidden state between adjacent stages, independent of pipeline depth. Block AttnRes requires all accumulated block representa tions at each stage for inter-block attention, and naïvely transmitting the full history at every transition incurs redundant communication.
...
$$
\text { Comm } _ {\text { cached }} = \underbrace {\frac {P (P - 1)}{2} N _ {p} d} _ {\text { first   virtual   stage }} + \underbrace {(V - 1) P ^ {2} N _ {p} d} _ {\text { subsequent   virtual   stages }}.\tag{8}
$$
...
3 $\{o_l^{(1)}, m_l^{(1)}, \ell_l^{(1)}\}_{l ∈ \mathcal{B}_n} \leftarrow \text{ATTNWITHSTATS}(Q, K, V)$ // Return LSE
4
/* Phase 2: Sequential intra-block attention + Online softmax merge */
5 i ← 0
6 for l ∈ B_n do
7 if i = 0 then
8 h_l ← o_l^{(1)}/ℓ_l^{(1)} // Inter-block only
9 else
10 o_l^{(2)}, m_l^{(2)}, ℓ_l^{(2)} ← ATTNWITHSTATS(w_l, b_n^i, b_n^i) // Intra-block
11 m_l ← max(m_l^{(1)}, m_l^{(2)})
12 h_l ← e^{m_l^{(1)} - m_l} o_l^{(1)} + e^{m_l^{(2)} - m_l} o_l^{(2)} // Online softmax merge
13 i ← i + 1
...
<table><tr><td rowspan="2" colspan="2"></td><td rowspan="2">Operation</td><td rowspan="2">Read</td><td rowspan="2">Write</td><td colspan="2">Total I/O</td></tr><tr><td>Symbolic</td><td>Typical</td></tr><tr><td colspan="2">Standard Residuals</td><td>Residual Merge</td><td>2d</td><td>d</td><td>3d</td><td>3d</td></tr><tr><td colspan="2">mHC (m streams)</td><td>Compute  $\alpha_l, \beta_l, A_l$ Apply  $\alpha_l$ Apply  $\beta_l$ Apply  $A_l$ Residual Merge</td><td>mdmd+m $d+m$  $md+m^2$ 2md</td><td> $m^2+2m$ mdmdmd</td><td>(8m+2)d+2 $m^2+4m$ </td><td>34d</td></tr><tr><td rowspan="2">AttnRes</td><td>Full</td><td>Phase 1 (amortized)Phase 2</td><td>(N-1)d(S-1)d</td><td>dd</td><td>(S+N)d</td><td>24d</td></tr><tr><td>Block</td><td>Phase 1 (amortized)Phase 2</td><td> $\frac{N}{S}d$ 3d</td><td>dd</td><td> $(\frac{N}{S}+5)d$ </td><td>5.5d</td></tr></table>
...
![](images/0181b98fc2d8025b5dca55bd676c4086101a0e9f72075b3eb9aad4c2bd3f8dc8.jpg)  
...
![](images/48e4b73a85994b97cde59d3746213b4e334e46797055d31d13dd99cd8e05b11f.jpg)
...
![](images/ffe5ada4e00263686ffa8702bb5bc0a704c2b190c075f77ce7af2d37c91dca1a.jpg)  
...
![](images/3b8cda78398a7dbe322c41f12d2b1b0897799eff587a1181d41615180e5a5f28.jpg)  
(a) Baseline

![](images/91d439dd7b4c4d2680025668f1c4ace4a8cd4f8b8788bdf2f5045b4d9cbbc224.jpg)  
...
![](images/d5a3e73b1f155ff0e55e7b5937c54e06836c817f067cf0c8ff1eb7632eb8ab70.jpg)  
...
<table><tr><td colspan="2">Method</td><td>Update rule</td><td>Weight</td><td>Source</td></tr><tr><td colspan="5">Single-state recurrence: layer l receives only  $\boldsymbol{h}_{l-1}$ </td></tr><tr><td colspan="2">Residual [12]</td><td> $\boldsymbol{h}_l = \boldsymbol{h}_{l-1} + f_{l-1}(\boldsymbol{h}_{l-1})$ </td><td>Fixed</td><td> $\boldsymbol{h}_{l-1}$ </td></tr><tr><td colspan="2">ReZero [2]</td><td> $\boldsymbol{h}_l = \boldsymbol{h}_{l-1} + \alpha_l \cdot f_{l-1}(\boldsymbol{h}_{l-1})$ </td><td>Static</td><td> $\boldsymbol{h}_{l-1}$ </td></tr><tr><td colspan="2">LayerScale [50]</td><td> $\boldsymbol{h}_l = \boldsymbol{h}_{l-1} + \text{diag}(\boldsymbol{\lambda}_l) \cdot f_{l-1}(\boldsymbol{h}_{l-1})$ </td><td>Static</td><td> $\boldsymbol{h}_{l-1}$ </td></tr><tr><td colspan="2">Highway [45]</td><td> $\boldsymbol{h}_l = (1-\boldsymbol{g}_l) \odot \boldsymbol{h}_{l-1} + \boldsymbol{g}_l \odot f_{l-1}(\boldsymbol{h}_{l-1})$ </td><td>Dynamic</td><td> $\boldsymbol{h}_{l-1}$ </td></tr><tr><td colspan="2">DeepNorm [54]</td><td> $\boldsymbol{h}_l = \text{Norm}(\alpha \boldsymbol{h}_{l-1} + f_{l-1}(\boldsymbol{h}_{l-1}))$ </td><td>Fixed</td><td> $\boldsymbol{h}_{l-1}$ </td></tr><tr><td colspan="2">KEEL [4]</td><td> $\boldsymbol{h}_l = \text{Norm}(\alpha \boldsymbol{h}_{l-1} + f_{l-1}(\text{Norm}(\boldsymbol{h}_{l-1})))$ </td><td>Fixed</td><td> $\boldsymbol{h}_{l-1}$ </td></tr><tr><td colspan="5">Multi-state recurrence: layer l receives m streams</td></tr><tr><td colspan="2">SiameseNorm [27]</td><td> $\boldsymbol{h}_l^1 = \text{Norm}(\boldsymbol{h}_{l-1}^1 + \boldsymbol{y}_{l-1}); \boldsymbol{h}_l^2 = \boldsymbol{h}_{l-1}^2 + \boldsymbol{y}_{l-1}$ </td><td>Fixed</td><td>2 streams</td></tr><tr><td colspan="2">HC/mHC [72, 59]</td><td> $\boldsymbol{H}_l = \boldsymbol{H}_{l-1} \boldsymbol{A}_l + f_{l-1}(\boldsymbol{H}_{l-1} \boldsymbol{\alpha}_{l-1}) \boldsymbol{\beta}_{l-1}^\top$ </td><td>Dynamic</td><td>m streams</td></tr><tr><td colspan="2">DDL [67]</td><td> $\boldsymbol{H}_l = (\mathbf{I} - \boldsymbol{\beta}_l \boldsymbol{k}_l \boldsymbol{k}_l^\top) \boldsymbol{H}_{l-1} + \boldsymbol{\beta}_l \boldsymbol{k}_l \boldsymbol{v}_l^\top$ </td><td>Dynamic</td><td> $d_v$  streams</td></tr><tr><td colspan="5">Cross-layer access: layer l can access individual earlier-layer outputs</td></tr><tr><td colspan="2">DenseNet [17]</td><td> $\boldsymbol{h}_l = \text{ConvPool}([ \boldsymbol{h}_1; f_1(\boldsymbol{h}_1); \ldots; f_{l-1}(\boldsymbol{h}_{l-1})])$ </td><td>Static</td><td> $[\boldsymbol{h}_1, \ldots, \boldsymbol{h}_{l-1}]$ </td></tr><tr><td colspan="2">DenseFormer [36]</td><td> $\boldsymbol{h}_l = \alpha_{0 \to l} \boldsymbol{h}_1 + \sum_{i=1}^{l-1} \alpha_{i \to l} f_i(\boldsymbol{h}_i)$ </td><td>Static</td><td> $[\boldsymbol{h}_1, \ldots, \boldsymbol{h}_{l-1}]$ </td></tr><tr><td colspan="2">MRLA [10] $^1$ </td><td> $\boldsymbol{h}_l = \sum_{i=1}^{l-1} \sigma(\text{ConvPool}(f_{l-1}(\boldsymbol{h}_{l-1})))^\top \sigma(\text{ConvPool}(f_i(\boldsymbol{h}_i))) \text{ Conv}(f_i(\boldsymbol{h}_i))$ </td><td>Dynamic</td><td> $[\boldsymbol{h}_1, \ldots, \boldsymbol{h}_{l-1}]$ </td></tr><tr><td rowspan="2">AttnRes (ours)</td><td>Full $^2$ </td><td> $\boldsymbol{h}_l \propto \sum_{i=0}^{l-1} \phi(\boldsymbol{w}_l, \boldsymbol{k}_i) \boldsymbol{v}_i$ </td><td>Dynamic</td><td> $[\boldsymbol{h}_1, \ldots, \boldsymbol{h}_{l-1}]$ </td></tr><tr><td>Block $^3$ </td><td> $\boldsymbol{h}_l \propto \sum_{i=0}^{n-1} \phi(\boldsymbol{w}_l, \boldsymbol{k}_i) \boldsymbol{v}_i + \phi(\boldsymbol{w}_l, \boldsymbol{k}_n^j) \boldsymbol{v}_n^j$ </td><td>Dynamic</td><td> $[\boldsymbol{b}_0, \ldots, \boldsymbol{b}_{n-1}, \boldsymbol{b}_n^j]$ </td></tr></table>
...
$$
\mathbf {W} _ {t} = \mathbf {W} _ {t - 1} - \eta \nabla \ell (\mathbf {W} _ {t - 1}; \pmb {x} _ {t}),\tag{9}
$$
...
![](images/6ea027ca501815c8fff551b646347c91726996e3d5da41ff5ff1181a487f242d.jpg)  
...
$$
\mathbf {H} _ {l} = \mathbf {H} _ {l - 1} \mathbf {A} _ {l} + f _ {l - 1} (\mathbf {H} _ {l - 1} \boldsymbol {\alpha} _ {l - 1}) \boldsymbol {\beta} _ {l - 1} ^ {\top},
$$
...
<table><tr><td>Guangyu Chen*</td><td>Yanru Chen</td></tr><tr><td>Yu Zhang*</td><td>Xin Men</td></tr><tr><td>Jianlin Su*</td><td>Haiqing Guo</td></tr><tr><td>Weixin Xu</td><td>Y. Charles</td></tr><tr><td>Siyuan Pan</td><td>Haoyu Lu</td></tr><tr><td>Yaoyu Wang</td><td>Lin Sui</td></tr><tr><td>Yucheng Wang</td><td>Jinguo Zhu</td></tr><tr><td>Guanduo Chen</td><td>Zaida Zhou</td></tr><tr><td>Bohong Yin</td><td>Weiran He</td></tr><tr><td>Yutian Chen</td><td>Weixiao Huang</td></tr><tr><td>Junjie Yan</td><td>Xinran Xu</td></tr><tr><td>Ming Wei</td><td>Yuzhi Wang</td></tr><tr><td>Y. Zhang</td><td>Guokun Lai</td></tr><tr><td>Fanqing Meng</td><td>Yulun Du</td></tr><tr><td>Chao Hong</td><td>Yuxin Wu</td></tr><tr><td>Xiaotong Xie</td><td>Zhilin Yang</td></tr><tr><td>Shaowei Liu</td><td>Xinyu Zhou</td></tr><tr><td>Enzhe Lu</td><td></td></tr><tr><td>Yunpeng Tai</td><td></td></tr></table>
...
$$
\mathrm{Write} _ {\mathrm{inter}} = L d\tag{13}
$$
...
$$
\operatorname{Read} _ {\text { total }} = d L (N - 1) + N \cdot S (S - 1) d, \quad \operatorname{Write} _ {\text { total }} = 2 L d.\tag{15}
$$
